...
The app supports multiple building scenarios via a dropdown selector in the header. Each scenario defines its own tab structure, data paths, fixture data, and building context.

### Scenario Infrastructure
- `src/scenarios/index.js` — Scenario configs (tabs, rates, ECMs, fixture data, data paths)
- `src/scenarios/context.js` — React context creation
- `src/scenarios/ScenarioContext.jsx` — Context provider component
- `src/scenarios/useScenario.js` — Hook to access current scenario
- `src/components/ScenarioSelector.jsx` — Dropdown in header

### Available Scenarios

| Scenario | Building | Approach | Tabs |
|----------|----------|----------|------|
| **Greenfield** (default) | 62k sf municipal center, CZ 4A | Inverse/statistical model | 7 tabs: Overview, Scatter, Model Fitting, Lighting, VFD, Time Series, Savings |
| **SLO Large Office** | 90k sf office, CZ 3C (California) | Calibrated simulation / forward model | 8 tabs: Overview, Energy Signature, Calibration, Sensitivity, Lighting, ECM Savings, Competent Counterfactual, MnVScore |

### SLO Large Office Scenario

**Building:** 90,000 sq ft, 5-story commercial office in San Luis Obispo, CA. 1980s VAV system with hot water reheat, centrifugal chiller (COP 3.2), gas-fired boiler. 15 thermal zones.

**ECMs:**
- ECM-1: LED Lighting Retrofit (LPD 10.8 → 7.0 W/m²) — Retrofit isolation / key parameter measurement
- ECM-2: Chiller Replacement (COP 3.2 → 4.0) — Whole facility / calibrated simulation
- ECM-3: VAV Box Optimization — Whole facility / calibrated simulation

**What SLO exercises that Greenfield doesn't:**
- Forward model calibration and ASHRAE G14 assessment
- Parameter sensitivity analysis (tornado chart, uncertainty propagation)
- Competent counterfactual evaluation (5-criteria checklist)
- MnVScore self-assessment (8-dimension rubric per ECM)
- Gas model limitation disclosure

**What SLO intentionally omits:**
- No student regression fitting (forward model, not inverse)
- No VFD analysis (no VFD ECM)
- No reporting period / time series overlay
- No NRA event (Greenfield's domain)

**SLO Data Files** (in `public/data/`):
| File | Source | Description |
|------|--------|-------------|
| `slo_baseline_monthly.csv` | Utility data + EnergyPlus OAT | 12 rows: month, days, avg_oat_f, total_kwh, total_therms |
| `slo_sensitivity_results.json` | EnergyPlus sensitivity runs | 8 parameters with ±15% impacts |
| `slo_ecm_savings.json` | Derived from sensitivity | 3 ECM projected savings with uncertainty |
| `slo_calibration_summary.json` | EnergyPlus calibration | NMBE 1.6%, CV(RMSE) 10.6%, parameter table |

**SLO Components:**
| Component | Tab | Purpose |
|-----------|-----|---------|
| `SloOverview.jsx` | Overview | Building context, ECMs, rates, gas limitation |
| `SloGraphic.jsx` | Overview | SVG 5-story office elevation |
| `CalibrationView.jsx` | Calibration Assessment | Simulated vs metered, G14 metrics, parameter docs |
| `SensitivityAnalysis.jsx` | Sensitivity Analysis | Tornado chart, parameter rankings, measurement priority |
| `ForwardModelSavings.jsx` | ECM Savings | Waterfall + bar charts with uncertainty bands |
| `CompetentCounterfactual.jsx` | Competent Counterfactual | Provenance table, 5-criteria checklist |
| `MnvScorePanel.jsx` | MnVScore | 8-dimension self-assessment, radar chart comparison |

**Reused Components (adapted via props/scenario):**
- `ScatterPlot.jsx` — Same component, different data (cooling-dominated vs heating-dominant)
- `LightingStipulation.jsx` — Accepts `fixtureData` and `electricRate` from scenario config
- `SummaryTable.jsx` — Accepts `sqft` prop (no longer hardcoded), handles baseline-only mode

**Paper Packet:** `scripts/build_slo_packet.py` generates 14-worksheet PDF (requires `reportlab`).

**Data Pipeline:** `scripts/prepare_slo_data.py` transforms EnergyPlus-MCP output data into app format. Source data lives in `~/EnergyPlus-MCP/energyplus-mcp-server/`
