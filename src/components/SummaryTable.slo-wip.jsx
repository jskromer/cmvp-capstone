export default function SummaryTable({ baseline, reporting, noNRA, showAnswerKey, sqft = 62000 }) {
  ...
  const hasReporting = reporting && reporting.length > 0;
  const rKwh = hasReporting ? sum(reporting, 'total_kwh') : 0;
  const rThm = hasReporting ? sum(reporting, 'total_therms') : 0;

  // Baseline-only mode (SLO scenario)
  if (!hasReporting) {
    return (
